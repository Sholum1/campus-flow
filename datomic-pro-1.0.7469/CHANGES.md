...
* This document covers all releases.  For a summary of critical
  ...
  https://docs.datomic.com/release-notices.html.
...
Datomic release 1.0.7469 fixes a bug that prevented some excisions from excising datoms from the history indexes (used in `d/as-of`, `d/since`, or `d/history` database values). This release also includes a tool to detect incomplete excisions of the history index and optionally re-apply them. This bug only affects databases that contain excision datoms (`:db/excise`), and does not affect the transaction log.

See https://docs.datomic.com/release-notices.html#excision-repair-tool.
